...
class BankActionModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # --- КРЕДИТ (ЗАПИТ ПІДТВЕРДЖЕННЯ) ---
            ...
            if amount > 50000: 
                return await interaction.response.send_message("❌ Максимальний кредит у банку — 50,000 AC.", ephemeral=True)
            ...
            await interaction.response.send_message(
                f"❓ Ви справді хочете взяти кредит у банку на суму **{amount} AC** під заставу казначейства?", 
                view=view, 
                ephemeral=True
            )
...
class RepayConfirmationView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.is_bank:
            ...
            debtor["bank_loan"] = {"amount": 0, "deadline": 0}
            msg = f"✅ <@{self.debtor_id}> повернув банку кредит: **{self.amount} AC**."
        else:
...
class LoanConfirmationView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.is_bank:
            ...
            borrower["bank_loan"] = {"amount": self.amount, "deadline": int(time.time()) + (7 * 86400)}
        else:
            ...
            borrower["active_loans"].append({
                "from_id": self.sender_id, "amount": self.amount, "deadline": int(time.time()) + (self.days * 86400)
            })
...
class EconomyCog(commands.Cog):
    ...
    def get_user(self, data, uid):
        ...
        if uid not in data:
            data[uid] = {
                "balance": 100, 
                "bank": 0, 
                "bank_loan": {"amount": 0, "deadline": 0},
                "active_loans": []
            }
        data[uid].setdefault("bank", 0)
        data[uid].setdefault("balance", 0)
        data[uid].setdefault("bank_loan", {"amount": 0, "deadline": 0})
    ...
    async def loan_checker_loop(self):
        ...
        for gid in os.listdir("server_data"):
            try:
                ...
                for uid in data:
                    u = data[uid]
                    loan = u.get("bank_loan", {"amount": 0, "deadline": 0})
                    if loan["amount"] > 0 and current_time > loan["deadline"]:
                        penalty = int(loan["amount"] * 1.5)
                        u["balance"] -= penalty
                        config["server_bank"] = config.get("server_bank", 0) + penalty
                        u["bank_loan"] = {"amount": 0, "deadline": 0}
                        updated = True
                if updated:
                    save_guild_json(guild_id, DATA_FILE, data)
                    save_guild_json(guild_id, ECONOMY_CONFIG, config)
    ...
    async def bank_menu(self, interaction: discord.Interaction):
        ...
        if user["bank_loan"]["amount"] > 0:
            embed.add_field(name="⚠️ Активний борг", value=f"`{user['bank_loan']['amount']} AC`", inline=False)
